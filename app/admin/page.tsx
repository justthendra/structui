...
export default function AdminDashboardPage() {
  ...
  const cards = [
    { label: "Total Developers", value: stats.totalUsers || 0, subtitle: "Registered accounts", icon: <Users className="w-5 h-5 text-blue-600" />, badge: "+100% all-time" },
    { label: "Published Snippets", value: stats.totalSnippets || 0, subtitle: "Code & templates", icon: <Code2 className="w-5 h-5 text-[#3D38E9]" />, badge: "0% Commission" },
    { label: "Verified Developers", value: stats.totalVerified || 0, subtitle: "Audited GitHub profiles", icon: <ShieldCheck className="w-5 h-5 text-emerald-600" />, badge: "Verified Badges" },
    ...
    { label: "Email Verified", value: stats.totalEmailVerified || 0, subtitle: "SMTP activated", icon: <MailCheck className="w-5 h-5 text-cyan-600" />, badge: "Security" },
    { label: "Banned / Restricted", value: stats.totalBanned || 0, subtitle: "Blocked violations", icon: <Ban className="w-5 h-5 text-rose-600" />, badge: "Moderation" },
  ];
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {cards.map((card, i) => (
          <motion.div
            ...
            className="flex flex-col rounded-3xl border border-neutral-200 bg-white p-2 hover:border-neutral-300 transition-all shadow-xs"
          >
            <div className="rounded-2xl border border-neutral-200/80 bg-neutral-50 p-5 flex flex-col justify-between h-[150px]">
              <div className="flex items-start justify-between">
                <div>
                  <span className="text-[11px] font-bold text-zinc-400 uppercase tracking-wider font-geist">
                    {card.label}
                  </span>
                  <p className="text-3xl font-extrabold font-bricolage text-[#202020] mt-1 tracking-tight">
                    {card.value}
                  </p>
                </div>
                <div className="p-2.5 bg-white rounded-xl border border-neutral-200/60 shadow-2xs">
                  {card.icon}
                </div>
              </div>

              <div className="flex items-center justify-between text-xs pt-2 border-t border-neutral-200/60">
                <span className="text-zinc-500 font-medium">{card.subtitle}</span>
                <span className="text-[10px] font-bold px-2 py-0.5 bg-white text-zinc-700 rounded-md border border-neutral-200">
                  {card.badge}
                </span>
              </div>
            </div>
                ...
                {recentUsers.length === 0 ? (
                  ...
                  recentUsers.map((u: any) => (
                    <tr key={u.id} className="hover:bg-neutral-50/60 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        {u.is_banned ? (
                          <span className="inline-flex items-center gap-1 text-rose-600 font-semibold bg-rose-50 px-2.5 py-0.5 rounded-full border border-rose-200">
