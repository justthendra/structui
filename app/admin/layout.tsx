...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  if (loading || !authorized) {
    return (
      <div className="min-h-screen bg-white dark:bg-[#09090b] text-[#202020] dark:text-[#f4f4f5] flex flex-col justify-between font-geist">
        <AdminNavbar />
        <div className="flex-1 flex items-center justify-center">
          <div className="flex flex-col items-center gap-3">
            <div className="size-9 border-3 border-[#3D38E9] border-t-transparent rounded-full animate-spin" />
            <p className="text-sm text-zinc-500 dark:text-zinc-400 font-medium">Verifying Admin Access…</p>
          </div>
        </div>
        <AdminFooter />
      </div>
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#09090b] text-[#202020] dark:text-[#f4f4f5] flex flex-col justify-between font-geist relative selection:bg-[#3D38E9]/20 selection:text-[#3D38E9] transition-colors duration-200">
      {/* Dedicated Admin Navbar */}
      <AdminNavbar />

      {/* Admin Hero Header with Dot Pattern */}
      <div className="relative bg-[#F7F7F7] dark:bg-zinc-950/70 border-b border-neutral-200/80 dark:border-zinc-800/80 pt-10 pb-8 px-6 overflow-hidden">
        ...
        <div className="max-w-[1200px] mx-auto relative z-10">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              {/* Badge */}
              <div className="inline-flex items-center gap-2 mb-2 bg-white dark:bg-zinc-900 px-3 py-1 rounded-full border border-neutral-200 dark:border-zinc-800 shadow-xs">
            ...
            <div className="flex items-center gap-1.5 overflow-x-auto pb-1 no-scrollbar bg-white dark:bg-zinc-900 p-1.5 rounded-2xl border border-neutral-200/80 dark:border-zinc-800 shadow-xs">
              {navTabs.map((tab) => {
                  ...
                  <Link
                    ...
                    className={`flex items-center gap-2 px-3.5 py-2 rounded-xl text-xs sm:text-sm font-semibold transition-all whitespace-nowrap cursor-pointer ${
                      active
                        ...
                        : "text-zinc-600 dark:text-zinc-300 hover:text-[#202020] dark:hover:text-white hover:bg-neutral-100/80 dark:hover:bg-zinc-800"
                    }`}
                  >
                    {tab.icon}
                    <span>{tab.label}</span>
