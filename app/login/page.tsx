...
export default function LoginPage() {
  ...
  React.useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (err) {
        if (err === "google_denied") setError("Google login was cancelled or denied.");
        else if (err === "google_not_configured") setError("Google login is not yet configured on the server.");
        else if (err === "google_token_failed" || err === "google_user_failed" || err === "google_error")
          setError("Failed to sign in with Google. Please try again.");
        else if (err === "discord_denied") setError("Discord login was cancelled or denied.");
        else if (err === "discord_not_configured") setError("Discord login is not yet configured on the server.");
